...
const Note = (props) => {
  const { note, id, selection, setSelection, setInspectorVisible, clef } = props
  const { x, y } = note.loc
  ...
  const color = () => (
      selection.id &&
      selection.id.measure===id.measure && 
      selection.id.note===id.note
    ) ? 'blue' : 'black'
  ...
  const renderNote = () => {
    ...
    if (!restPresent) {
      ...
      // render stem hook if eighth note
      if (type==='eighth') {
        if (step < 0) {
          glyphs.push(<path key={keyGenerator()} className="Hook" style={{fill: color()}} transform={`matrix(0.992126,0,0,0.992126,${551.4+x},${349-step*12.4016+y})`} d="M3.5,3.5 C3.40625,3.20313 2.5,-0.296875 2.5,-0.5 C2.40625,-0.90625 1.79688,-1.20313 1.29688,-1.20313 C0.59375,-1.20313 0,-0.59375 0,0.09375 L0,16.5 L0,22.7031 C0,23.2031 0.40625,23.5938 0.796875,23.7969 C5,25.0938 13.7031,30.6094 20.2031,41.7031 C22,44.7969 24.9063,49 24.9063,57.4063 C24.9063,64.6094 23.7969,72.2031 22,79.2031 C21.7031,80.3125 21.4063,81.3125 22.5,81.7031 C23.5938,82.1094 24,81.3125 24.4063,80.3125 C28,71.5 28.9063,62.2969 28.9063,56.1094 L28.9063,54.2031 C28.4063,41.2969 21,31.7969 21,31.7969 C21.2969,31.7969 11.7969,19.5 9.09375,15.2969 C5.40625,9.59375 3.59375,3.90625 3.5,3.5"/>)
        } else {
          glyphs.push(<path key={keyGenerator()} className="Hook" style={{fill: color()}} transform={`matrix(0.992126,0,0,0.992126,${521.863+x},${520.8264-step*12.4016+y})`} d="M2.5,0.40625 C2.5,0.203125 3.40625,-3.20313 3.5,-3.5 C3.59375,-3.90625 5.40625,-9.59375 9.09375,-15.2969 C11.7969,-19.5 21.2969,-31.7969 21,-31.7969 C21,-31.7969 28.4063,-41.2969 28.9063,-54.2031 L28.9063,-56.5 C28.9063,-62.6094 28.1094,-71.2031 24.5938,-79.9063 C24.2031,-80.9063 23.7969,-81.9063 22.7031,-81.5 C21.5938,-81.1094 21.9063,-79.9063 22.2031,-78.8125 C24,-71.7969 25,-64.6094 25,-57.4063 C25,-49 22,-44.7969 20.2031,-41.7031 C13.5938,-30.6094 5,-25.2031 0.796875,-23.7969 C0.296875,-23.5938 0,-23.2031 0,-22.7031 L0,-16.5 L0,-0.09375 C0,0.59375 0.59375,1.20313 1.29688,1.20313 C1.79688,1.20313 2.40625,0.796875 2.5,0.40625"/>)
        }
      }
